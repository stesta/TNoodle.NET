...
namespace cs.sq12phase
{
    public class Search
    {
        ...
        public static readonly int FACE_TURN_METRIC = 0;
        public static readonly int WCA_TURN_METRIC = 1;
        public static readonly int METRIC = WCA_TURN_METRIC; // only available for optimal solver

        private static readonly int PRUN_INC = METRIC == WCA_TURN_METRIC ? 2 : 1;
        ...
        static int count0xf(int val)
        {
            val &= val >> 1;
            val &= val >> 2;
            return Replacements.BitCount(val & 0x11111111);
        }

        bool phase1Opt(int shape, int prunvalue, int maxl, int depth, int lm, int lastTurns)
        {
            ...
            if (lm != 0)
            {
                int _shapex = Shape.TwistMove[shape];
                int prun = Shape.ShapePrunOpt[_shapex];
                if (prun < maxl / PRUN_INC)
                {
                    move[depth] = 0;
                    int next_maxl = (maxl / PRUN_INC - 1) * PRUN_INC;
                    if (phase1Opt(_shapex, prun, next_maxl, depth + 1, 0, lastTurns << 8))
                    {
                        return true;
                    }
                }
        ...
        bool idaPhase1(int shape, int prunvalue, int maxl, int depth, int lm)
        {
        ...
        bool initPhase2()
        {
            ...
            Replacements.assert(Shape.ShapePrun[d.getShapeIdx()] == 0);
            d.getSquare(sq);
        ...
        bool idaPhase2(int edge, int corner, bool topEdgeFirst, bool botEdgeFirst, int ml, int maxl, int depth, int lm)
        {
            if (maxl == 0 && !topEdgeFirst && botEdgeFirst)
            {
                Replacements.assert(edge == 0 && corner == 0 && ml == 0);
                return true;
            }
