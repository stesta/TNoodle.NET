...
namespace cs.min2phase
{
    public class CubieCube
    {
        ...
        public static sbyte[] MPermInv = new sbyte[CoordCube.N_MPERM];
         ...
         */
        // static sbyte[] e2c = {0, 0, 0, 0, 1, 3, 1, 3, 1, 3, 1, 3, 0, 0, 0, 0};
        public static readonly int SYM_E2C_MAGIC = 0x00DDDD00;
        ...
        public void invCubieCube()
        {
            ...
            {
                temps.ca[ca[corn] & 0x7] = (sbyte)(corn | 0x20 >> (ca[corn] >> 3) & 0x18);
            }
            copy(temps);
        }
        ...
        public static void CornMult(CubieCube a, CubieCube b, CubieCube prod)
        {
            for (int corn = 0; corn < 8; corn++)
            {
                ...
                prod.ca[corn] = (sbyte)(a.ca[b.ca[corn] & 7] & 7 | (oriA + oriB) % 3 << 3);
            }
        }
        ...
        public long selfSymmetry()
        {
            ...
            for (int i = 0; i < 96; i++)
            {
                ...
                if (Enumerable.SequenceEqual(d.ca, ca))
                {
                    EdgeConjugate(c, SymMultInv[0, i % 16], d);
                    if (Enumerable.SequenceEqual(d.ea, ea))
                    {
        ...
        public static void initSym()
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    ...
                    for (int k = 0; k < 16; k++)
                    {
                        if (Enumerable.SequenceEqual(CubeSym[k].ca, c.ca))
                        {
            ...
            for (int j = 0; j < 18; j++)
            {
                for (int s = 0; s < 16; s++)
                {
                    ...
                    for (int m = 0; m < 18; m++)
                    {
                        if (Enumerable.SequenceEqual(moveCube[m].ca, c.ca))
                        {
                            SymMove[s, j] = m;
        ...
        public static int initSym2Raw(int N_RAW, char[] Sym2Raw, char[] Raw2Sym, char[] SymState, int coord)
        {
            ...
            for (int i = 0; i < N_RAW; i++)
            {
                ...
                for (int s = 0; s < 16; s += sym_inc)
                {
                    ...
                    if (idx == i)
                    {
                        // HACK: not sure about this java -> csharp conversion
